...
#include "dropwell_plugin_private.h"
...
static void dropwell_plugin_handle_method_call(
    DropwellPlugin* self,
...
static void method_call_cb(FlMethodChannel* channel, FlMethodCall* method_call,
                           gpointer user_data) {
  DropwellPlugin* plugin = DROPWELL_PLUGIN(user_data);
  dropwell_plugin_handle_method_call(plugin, method_call);
}

void dropwell_plugin_register_with_registrar(FlPluginRegistrar* registrar) {
  DropwellPlugin* plugin = DROPWELL_PLUGIN(
      g_object_new(dropwell_plugin_get_type(), nullptr));
