...
void main() {
  ...
  final cases =
      (reference['tests']! as List<Object?>)
          ...
          .where(
            (entry) =>
                entry['file'] == 'test/playwright/InputHandler.test.ts' &&
                (entry['fullName']! as String).contains(
                  'CSI CSI Pm m - SGR: Character Attributes',
                ),
          )
          .toList()
        ..sort(
...
''');
  for (final entry in cases) {
    ...
    source
      ...
      ..writeln('    () => verifyInputHandlerSgrPlaywrightCase(');
  ...
  source.writeln('}');
  File(
    '${package.path}/test/upstream_input_handler_sgr_playwright_parity_test.dart',
  ...
  for (final entry in cases) {
    ...
    additions
      ...
      ..write(
        '"dartTestFile":'
        '"test/upstream_input_handler_sgr_playwright_parity_test.dart",',
      )
      ..write(
